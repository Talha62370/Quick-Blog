...
const Footer = () => {
  return (
    <div className="py-6 md:px-16 lg:px-24 xl:px-32 bd-primary/3">
      <div
      ...
      >
        ...
        <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5">
             {footer_data.map((section, index)=> (
                <div key={index}>
                    <h3 className="font-semibold text-base text-gray-900 mdmb-5 mb-2">
                        {section.title}
                    </h3>
                    <ul className="text-sm space-y-1">
                        {section.links.map((link, i)=> (
                            <li key={i}>
                                 <a href='#' className="hover:undeline transition">
                                    {link}
                                </a>
                            </li>
                        ))}
      ...
      <p className="py-4 text-center text-sm md:text-base text-gray-500">
        Copyright 2025 © QuickBlog - All Right are Reserved.
      </p>
    </div>
  );
};
